...
### 1. Source Code (소스 코드)
이 프로젝트의 소스 코드(`*.py`, `*.json`, `*.bat` 등 로직 파일)는 **MIT License** 하에 배포됩니다.
누구나 자유롭게 수정, 배포 및 사용할 수 있습니다.

### 2. Game Assets (게임 에셋)
`resources/` 폴더 내의 이미지 파일 및 게임 관련 데이터는 **MIT License의 적용 범위에서 제외**됩니다.
해당 자료의 저작권은 **Yostar Inc.** 및 원작자에게 있으며, 본 프로젝트에서는 '인식(Matching)'을 위한 참조 목적으로만 포함되어 있습니다.
사용자는 이 에셋 파일들을 상업적 목적으로 사용하거나, 원작자의 권리를 침해하는 방식으로 재배포해서는 안 됩니다.
